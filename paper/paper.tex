...
Henceforth I shall use the term ``sequent judgement'' to mean a
hypothetico-general judgement which also requires functionality. First, before
we get to the sequent judgement, we will need the concept of an ``open
categorical judgement''; this is a judgement which has variables from a
telescope free in it.

\subsection{Codes for Open Judgements}
We will define \emph{codes} for such judgements and then
interpret them into proper object judgement (LF-types) later.
...
extending the LF signature with a new judgement
